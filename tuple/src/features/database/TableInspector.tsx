...
export function TableInspector({ table }: { table: TableSchema | null }) {
  if (!table) return <aside className="hidden w-[296px] shrink-0 border-l border-border/80 bg-background xl:flex"><Empty><EmptyHeader><EmptyMedia><Table2 className="size-4"/></EmptyMedia><EmptyTitle>Select a table</EmptyTitle><EmptyDescription>Column details and relationships appear here.</EmptyDescription></EmptyHeader></Empty></aside>
  ...
  return <aside className="hidden w-[296px] shrink-0 border-l border-border/80 bg-background xl:flex xl:min-h-0 xl:flex-col">
    <div className="flex h-14 shrink-0 items-center gap-2.5 border-b border-border/80 px-3"><span className="grid size-8 place-items-center rounded-lg border border-border bg-card shadow-[0_1px_2px_rgb(0_0_0/0.05)]">{table.kind === "view" ? <Eye className="size-3.5 text-relationship"/> : <Table2 className="size-3.5 text-primary"/>}</span><div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold tracking-[-0.01em]">{table.name}</p><p className="text-[9px] text-muted-foreground">{table.schema} · {table.kind}</p></div><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon-sm" aria-label="Table actions"><MoreHorizontal/></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onSelect={() => void copyName()}><Copy/>Copy qualified name</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div>
    <div className="p-3"><Card className="grid grid-cols-2 divide-x divide-border overflow-hidden rounded-lg"><Metric label="Columns" value={table.columns.length}/><Metric label="Rows" value={table.rowCount == null ? "—" : new Intl.NumberFormat().format(table.rowCount)}/></Card></div>
