...
export function ResultGrid({ result, loading = false, editable = false, table, onUpdateCell }: ResultGridProps) {
  ...
  return <div className="flex h-full min-h-0 flex-col">
    <div className="flex h-11 shrink-0 items-center gap-2 border-b border-border bg-background px-3">
      ...
      <span className="font-mono text-[9px] text-muted-foreground/60">in {formatDuration(result.durationMs)}</span>
      ...
      <div className="ml-auto flex items-center gap-1">
        <InputGroup className="mr-1 hidden h-7 w-44 sm:flex"><InputGroupAddon className="pl-2"><Search /></InputGroupAddon><InputGroupInput value={filter} onChange={(event) => setFilter(event.target.value)} className="pl-1 text-[11px]" placeholder="Filter results…" aria-label="Filter results" autoComplete="off" /></InputGroup>
        <ButtonGroup>
    ...
    <ScrollArea className="min-h-0 flex-1">
      <Table>
        <TableHeader className="sticky top-0 z-10 bg-control"><TableRow><TableHead className="w-12 text-right">#</TableHead>{result.columns.map((column) => <TableHead key={column}><span className="flex items-center gap-1.5"><Filter className="size-2.5 opacity-35"/>{column}</span></TableHead>)}</TableRow></TableHeader>
