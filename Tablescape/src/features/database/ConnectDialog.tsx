...
import { chooseDatabaseFile, chooseSqlFile, connectDatabase, importSqlFile, isNativeApp } from "@/lib/database-api"
...
export function ConnectDialog({ open, onOpenChange, onConnected }: ConnectDialogProps) {
  ...
  const importSql = async () => {
    ...
    if (!path) {
      if (!native) toast.info("SQL import is available in the Tauri desktop app.")
      return
    }
    setPending(true)
    try {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[calc(100vh-1.5rem)] max-w-2xl gap-0 overflow-auto p-0">
        ...
        <div className="grid gap-5 p-6">
          {!native ? <div role="status" className="border-l-2 border-primary bg-primary/8 px-3 py-2.5 text-xs leading-5 text-foreground"><span className="font-semibold">Browser preview:</span> connection controls are visible for interface testing, but live database access and file picking require the Tauri desktop app.</div> : null}
          <Tabs value={source} onValueChange={(value) => { setSource(value as ConnectionSource); setLocation("") }}>
            <TabsList className="grid h-auto w-full grid-cols-2 gap-1 bg-muted/55 p-1">
              {options.map((option) => <TabsTrigger key={option.source} value={option.source} className="h-10 flex-col items-start gap-0.5 px-3 text-left sm:h-14"><span className="flex w-full items-center gap-2 text-sm"><option.icon className="size-3.5" />{option.label}</span><span className="hidden w-full font-normal text-muted-foreground sm:block">{option.detail}</span></TabsTrigger>)}
            </TabsList>
          </Tabs>
          ...
          <Button type="button" variant="outline" disabled={pending} onClick={() => void importSql()} className="group h-auto justify-start gap-3 border-dashed bg-muted/20 p-3 text-left hover:border-primary/50 hover:bg-primary/5">
            <span className="grid size-9 place-items-center border border-border bg-background"><FileCode2 className="size-4 text-muted-foreground" /></span>
