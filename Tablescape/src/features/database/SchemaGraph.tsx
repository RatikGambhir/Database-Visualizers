...
function GraphToolbar({ onResetLayout }: { onResetLayout: () => void }) {
  ...
  return <Card className="flex items-center gap-1 border-[var(--line-strong)] bg-popover p-1 shadow-[0_10px_28px_-18px_rgb(0_0_0/0.5)]">
    <ButtonGroup>
...
export function SchemaGraph({ schema, selectedTable, onSelectTable, onOpenTable }: SchemaGraphProps) {
  ...
  if (schema.tables.length === 0) return <div className="grid h-full place-items-center bg-canvas"><Card className="max-w-sm px-6 py-8 text-center"><Waypoints className="mx-auto size-5 text-muted-foreground"/><p className="mt-3 text-sm font-semibold">No schema objects found</p><p className="mt-1 text-xs leading-5 text-muted-foreground">This database has no visible tables or views. Create an object, then refresh the schema.</p></Card></div>
  ...
  return <div className="relative h-full min-h-0 bg-canvas">
    ...
    >
      ...
      <Panel position="top-left" className="!m-4">
        <Card className="pointer-events-none flex items-center gap-2.5 border-[var(--line-strong)] bg-popover px-3 py-2">
          <span className="grid size-7 place-items-center border-r border-primary bg-primary/10 text-primary"><Waypoints className="size-3.5" /></span>
          <div>
            <p className="text-[11px] font-semibold">Schema map</p>
            <p className="font-mono text-[8px] uppercase tracking-[0.06em] text-muted-foreground">{schema.tables.length} objects / drag to arrange / double-click to browse</p>
          </div>
        </Card>
      </Panel>
      <MiniMap
        className="!bottom-4 !right-4 !h-28 !w-40 !rounded-[3px] !border !border-[var(--line-strong)] !bg-popover !shadow-none"
