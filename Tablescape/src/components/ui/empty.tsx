...
function EmptyMedia({ className, variant = "icon", ...props }: React.ComponentProps<"div"> & { variant?: "icon" | "plain" }) {
  return <div data-slot="empty-media" data-variant={variant} className={cn("mb-1 flex items-center justify-center text-muted-foreground data-[variant=icon]:size-10 data-[variant=icon]:rounded-[3px] data-[variant=icon]:border data-[variant=icon]:border-border data-[variant=icon]:bg-card", className)} {...props} />
}
