...
import type { ConnectionInput, ConnectionProfile, DatabaseSchema, QueryResult } from "@/lib/types"
...
export async function connectDatabase(config: ConnectionInput): Promise<{ connection: ConnectionProfile; schema: DatabaseSchema }> {
  if (!inTauri()) {
    await pause()
    return {
      connection: { id: crypto.randomUUID(), name: config.name, kind: config.kind, detail: config.path ?? config.url ?? "Local database", connected: true, readOnly: config.readOnly },
